...
import { clientName, fetchFriends, mechanism, rxep, setupSession, xmlDeclaration, acceptFriendRequest, sendMessage } from "./xml-objects";
...
export class ValorantXmppClient extends EventEmitter {
        ...
        try {
            this._presenceInterval = setInterval(async () => { await this.sendPresence(); },
                typeof this._config.updatePresenceInterval === 'number'
                    ...
                    : this._config.updatePresenceInterval()) as NodeJS.Timer;
            ...
            clearInterval(this._presenceInterval as NodeJS.Timeout);
    ...
    acceptFriendRequest = async (puuid: string) => {
        await (await this.getXmppInstance()).sendXml(acceptFriendRequest(puuid));
    }
    sendMessage = async (jid: string, content: string) => {
        await ((await this.getXmppInstance()).sendXml((sendMessage(content, jid, this.tokenStorage.region.domain))));
    }
    end = () => {
